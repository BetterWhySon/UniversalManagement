...
interface Props {
  onClose: () => void;
  onSave: (data: typeAdmModelCustomAlarmList) => void;
  initialData?: typeAdmModelCustomAlarmList;
}

export default function ManufacturerAlarmRegistrationPopup({ onClose, onSave, initialData }: Props) {
  const { t: trans } = useTranslation('translation');
  const customerId = useCustomerId();
  const [formData, setFormData] = useState<Partial<typeAdmModelCustomAlarmList>>({
    name: '',
    type: 0,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...formData,
      referred_manufacturer: Number(customerId)
    } as typeAdmModelCustomAlarmList);
  };
      ...
      <div className="bg-hw-dark-2 rounded-lg w-full max-w-[500px] mx-4">
        <div className="p-6">
          <h2 className="text-xl text-white mb-6">
            {initialData ? trans('알람 데이터 수정') : trans('알람 데이터 등록')}
          </h2>
          <form onSubmit={handleSubmit}>
            <div className="space-y-4">
              <div>
                <label className="block text-white mb-2">{trans('알람 데이터명')} *</label>
                <input
                  type="text"
                  value={formData.name}
                  ...
                  placeholder={trans('알람 데이터명을 입력하세요')}
                />
              </div>
              <div>
