...
const BatteryAlarm: React.FC = () => {
  ...
  const getTooltipText = (type: 'normal' | 'alarm') => {
    if (type === 'normal') {
      return '관리차량 中 안전상의 문제가 없는 차량 대수';
    }
    return '관리차량 中 배터리 이상이 의심되는 관리 차량 수. 배터리 진단 알고리즘을 적용해 이상이 감지된 차량을 식별하여, 잠재적인 고장이나 안전 위험에 대해 선제적으로 관리.';
  };

  return (
    <div className="bg-gray-800 p-3 rounded-lg border border-white h-full flex flex-col overflow-hidden">
      <h3 className="text-white text-lg mb-4 text-left">진행중인 배터리 이상알람</h3>
