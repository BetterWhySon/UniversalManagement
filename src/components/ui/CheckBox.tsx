...
export default function CheckBox({ label, description, gap, checked, fontSize = '16px', fontWeight = '300', index }: CheckBoxProps) {
  const [isChecked, setIsChecked] = useState(checked);
  const { detailDataTypes, storeCheck, storeUnCheck } = useSystemDetailDataType();

  const toggleCheckbox = () => {
    if( detailDataTypes[index+1].checked ) {
      storeUnCheck(index+1) 
    } else {
      storeCheck(index+1) 
    }
    // detailDataTypes[index+1].checked = !detailDataTypes[index+1].checked
  ...
  useEffect(() => {   // 생성,수정,삭제 후 데이터 리로드 처리
    setIsChecked(detailDataTypes[index+1].checked);
  },[detailDataTypes, index]);
